...
if [ $? -eq 0 ]; then
    ...
    if [ -f "core_cams_method_selection_results.json" ]; then
        echo "Results file created: core_cams_method_selection_results.json"
        echo "File size: $(du -h core_cams_method_selection_results.json | cut -f1)"
