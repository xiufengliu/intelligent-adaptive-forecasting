...
from .cams_pipeline import CamsPipeline
...
def run_core_method_selection_experiment():
    """Run ONLY the core CaMS method selection experiment for Table 3"""
    ...
    try:
        ...
        # Train CaMS using ONLY real data
        logger.info("Training CaMS model using ONLY real datasets...")
        logger.info("NO synthetic data, NO data augmentation - maintaining academic integrity")
        
        # Prepare training data
        # Note: CaMS extracts features internally; we prepare raw windows and labels
        ...
        # Train the model
        training_results = pipeline.train_cams_model()
        logger.info("CaMS training completed")
        
        # Evaluate CaMS method selection on each dataset
        ...
        for dataset_name, dataset_info in pipeline.datasets.items():
            logger.info(f"Testing CaMS selection on {dataset_name}...")
        ...
        # Print summary
        logger.info("=" * 60)
        logger.info("🎯 CORE CaMS METHOD SELECTION RESULTS")
        logger.info("=" * 60)
...
if __name__ == "__main__":
    ...
    print("🎉 Core CaMS method selection experiment completed!")
