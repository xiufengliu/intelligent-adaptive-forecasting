...
Includes ONLY: Individual Methods, Selection Methods (Oracle, Random, FFORMA, Rule-based), CaMS
...
def load_fresh_cams_results():
    """Load FRESH CaMS results from current experiment - NO cached data"""
    logger = logging.getLogger(__name__)

    # Check file modification time to ensure freshness
    cams_file = 'core_cams_method_selection_results.json'

    try:
        # Check if file exists and get modification time
        if not os.path.exists(cams_file):
            logger.error(f"❌ CRITICAL: CaMS results file not found: {cams_file}")
            raise FileNotFoundError("Fresh CaMS results required - no synthetic data allowed")

        # Get file modification time
        mod_time = os.path.getmtime(cams_file)
        mod_datetime = datetime.fromtimestamp(mod_time)
        logger.info(f"📅 CaMS results file last modified: {mod_datetime}")

        # Load the results
        with open(cams_file, 'r') as f:
            cams_results = json.load(f)

        # Verify it's a legitimate experiment
        if 'experiment_info' not in cams_results:
            raise ValueError("Invalid CaMS results format")

        exp_info = cams_results['experiment_info']
        if exp_info.get('synthetic_data_used', True):
            raise ValueError("CaMS results contain synthetic data - cannot use")

        summary = cams_results['method_selection_results']['summary']

        logger.info("✅ Loaded FRESH CaMS results from current experiment")
        ...
        logger.info(f"📊 CaMS accuracy: {summary['selection_accuracy']:.1%}")
        logger.info(f"📊 CaMS MASE: {summary['avg_mase']:.3f}")
        logger.info(f"📊 CaMS diversity: {summary['method_diversity']} methods")
        ...
        logger.error("❌ CRITICAL: Fresh CaMS results not found!")
        logger.error("Academic integrity requires fresh experimental results only.")
        raise FileNotFoundError("Fresh CaMS results required for academic integrity - no synthetic data allowed")
    except Exception as e:
        logger.error(f"❌ Error loading fresh CaMS results: {str(e)}")
        raise
...
def run_comprehensive_baseline_comparison():
    ...
    logger.info("Scope: Individual Methods + Selection Methods + CaMS (FRESH DATA ONLY)")
    ...
    try:
        ...
        # Load FRESH CaMS results (NO cached data) - Optional
        logger.info("📊 Loading FRESH CaMS results...")
        try:
            cams_result = load_fresh_cams_results()
        except FileNotFoundError:
            logger.warning("⚠️ CaMS results not found - creating placeholder")
            cams_result = {
                'category': 'CaMS (Ours)',
        ...
        all_results = {
            ...
            'summary': {
                ...
                'cams_mase': cams_result['avg_mase']
            }
        }
        ...
        logger.info("📊 CaMS (FRESH DATA):")
        logger.info(f"  {cams_result['method']}: {cams_result['avg_mase']:.3f} MASE ({cams_result['selection_acc']} acc)")
        logger.info(f"  CaMS timestamp: {cams_result.get('timestamp', 'Unknown')}")
